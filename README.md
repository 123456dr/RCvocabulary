...
---
10/26<br>
1. 修正開啟待學習單字庫開關的狀態下，關閉app或跳轉頁面造成途中learned的單字無法復原初始題庫(原先設計:切換成待學習題庫練習後的資料更動不會影響到位切換時的初始題庫)
2. 修正當日單字計數顯示，更改運作為:當下時刻與上一次紀錄時刻不同即歸零(只記錄小時制)

10/16<br>
* 新增切換背景底色按鈕:暗色系讓眼睛更舒服，同時切換文字顏色(避免不清楚)
* 優化ANS:當按下第三次意即在隱藏答案前把正確單字填入輸入框，讓使用者可以直接按確認(情境:我已經會了，不想再輸入一次浪費時間=>直接按按鈕解決)
* 優化句子模式:解決按下skip單字更新句子卻沒有更新
* 測驗模式:
   - 當使用者輸入錯誤的單字時(拼錯)自動發音
   - 自動導向輸入框:當使用者按下skip或learn或enter，直接導向輸入框繼續做輸入(原先:需再次點選輸入框才顯示鍵盤)
* 待新增:收藏句子,分享句子,複製文字
